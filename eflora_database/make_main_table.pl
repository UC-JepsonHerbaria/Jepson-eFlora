...
tie(%IJM_key, "BerkeleyDB::Hash", -Filename=>"/JEPS-master/Jepson-eFlora/eflora_keys/output/IJM_key.hash", -Flags      => BerkeleyDB::DB_CREATE )|| die "Stopped; couldnt open IJM_key\n";

#process the bioregion hex code file, generated by get_all_dist.pl
open(IN, "output/nomsyn_HCODE_cch_out.txt") || die;
while(<IN>){
	$hex_code="";
	chomp;
...
elsif ($taxon_name =~ /([A-Z]+) ([a-z-]+) (.+) (nothosubsp.|subvar.|var\.|subsp\.) ([a-z-]+)$/){ #if it is an infra with an author inserted (that's the ".+")
	$genus = ucfirst(lc($1));
	$species = $2;
	$species_author = $3;
	$infra_rank = $4;
...
elsif ($taxon_name =~ /([A-Z]+) ([a-z-]+) (nothosubsp.|subvar.|var\.|subsp\.) ([a-z-]+)$/){ #if it is an infra without an author in the name, except if we have forma, those should go through strip name below
	$genus = ucfirst(lc($1));
	$species = $2;
	$infra_rank = $3;
	$infra_epithet = $4;
