...
#process the bioregion hex code file, generated by get_all_dist_local.pl #use the cumulative hash file below, instead of this text file
...
open(IN, "/JEPS-master/Interchange/input/CNPS/CNPS_ID.txt") || die;
while(<IN>){
	$CNPS_ID="";
	$CNPS_name="";
	s/X /&times;/;
...
#$dist_code=$HEX{$taxon_id};
	if ($HEX{$taxon_id}){
		...
		$dist_array = "NULL";
		$dist_code = "NULL";
		#print "HCODE PROBLEM: $taxon_id,$name_for_matching";
	}
