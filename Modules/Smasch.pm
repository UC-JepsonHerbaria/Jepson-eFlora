...
use Exporter;
@ISA = ('Exporter');
@EXPORT = qw(&strip_name @S_accession %S_accession_length %S_accession @S_folder %S_folder_length %S_folder %vouchers %magic_no @month &process_name &verify_bioname &get_TRS &get_elev &parse_lat &parse_long &load_data &load_cal &load_be &load_noauth_name %seen %datafile %coll_comm &load_collectors %PARENT &verify_cal_long &verify_cal_lat);
####patterns######
$genus = '[A-Z][a-z-]+';
$species = '[a-z-]+';
...
$author = '.*';
########
$tnoan_out="/Users/jfp04/CDL_buffer/buffer/tnoan.out";
sub verify_cal_lat {
	local($_)=shift;
	local($v_dec,$v_deg)=&parse_lat($_);
	return 0 if $v_dec > 42.1;
	return 0 if $v_dec < 32.5;
	return 1;
}
sub verify_cal_long {
	local($_)=shift;
	local($v_dec,$v_deg)=&parse_long($_);
	#warn "DEC $v_dec  DEG $v_deg\n";
	return 0 if $v_dec > -114;
	return 0 if $v_dec < -124.5;
	return 1;
}
